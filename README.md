...
- 📝 **歌词显示**：支持内嵌歌词与同名 `.lrc` 文件（自动识别 UTF-8 / GBK），逐行高亮、自动滚动、点击行跳转
- 🔀 随机播放 / 单曲循环 / 列表循环
- ⏱ **睡眠定时器**（10-90 分钟）
- ⚡ **倍速播放**（0.5x - 2.0x）
- ❤️ 收藏 + 自定义播放列表
...
- **WMA / APE**：会在曲库中正常显示；能否直接播放取决于设备解码器 —— 大多数现代 Android 设备已移除 WMA 解码器，播放时 LunaMusic 会**自动跳到下一首并给出提示**。如需完整体验，建议将 WMA 转码为 MP3/FLAC（原生 FFmpeg 扩展方案在 Roadmap 中）
...
- [ ] 通过 FFmpeg 原生扩展支持 WMA/APE 直接解码播放
- [ ] 文件夹浏览模式
- [ ] 桌面小组件
- [ ] WAV/APE/FLAC 高清格式图标标识与更多元数据（采样率/位深）
- [ ] 歌词翻译行、卡拉OK式逐字歌词
- [ ] 多语言（English）
